...
class Generator(object):
    ...
    def generate(self, random_data = True):
        ...
        while True:
            ...
            # 从文件中读取图像
            png1 = Image.open(r"./VOCdevkit/VOC2007/JPEGImages" + '/' + name + ".png")
            png = Image.open(r"./VOCdevkit/VOC2007/SegmentationClass" + '/' + name + ".png")

            if random_data:
                png1, png = self.get_random_data(png1,png,(int(self.image_size[1]),int(self.image_size[0])))
            else:
                png1, png = letterbox_image(png1, png, (int(self.image_size[1]),int(self.image_size[0])))
            
            inputs.append(np.array(png1)/255)
